...
def init_coin_tracking(coin):
    last_alert[coin] = {tf: {"ema": None, "rsi": None, "bb": None} for tf in timeframes}
...
def get_prices(symbol, interval):
    try:
        r = requests.get(
            ...
            params={"category":"linear","symbol":symbol,"interval":interval,"limit":200}, timeout=10
        )
        data = r.json()
...
# ----------------- START -----------------
send_alert("🚨 BOT WITH SMART ALERTS RUNNING 🚨")
